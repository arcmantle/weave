...
/**
 * DOM JSX typing surface for lit-jsx.
 *
 * This file intentionally starts at the split point (`type EventHandler...`).
 * It relies on a few shared core aliases defined in `jsx-core.ts`:
 * - `LitJSX.DOMElement`
 * - `LitJSX.Child`
 * - `LitJSX.CanBeNothing`
 *
 * Keep those shared types in `jsx-core.ts` to avoid duplicate global `JSX` declarations.
 *
 * Based on SolidJS JSX types; adapted for Lit-html.
 */
...
declare global {
	namespace LitJSX {
		...
		type InputEventHandler<T, E extends InputEvent> = (e: E & {
			currentTarget: T;
			target:        T extends HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement ? T : LitJSX.DOMElement;
		}) => void;
		...
		//#region Custom attributes & DOM attribute base types
		...
		//#region HTML attributes (base + element-specific)
		// NOTE: Large type surface; keep edits minimal to avoid accidental breaking changes.
		...
		interface HTMLAttributes<T> extends
			...
			/**
			 * Legacy attribute: tells the compiler to compile this Functional Component
			 * using its tag as a static literal.
			 */
			//static?: boolean;
		}
		...
		//#region HTML element-specific attributes
		...
		//#region Tag name -> attribute map (used by `JSX.IntrinsicElements`)
