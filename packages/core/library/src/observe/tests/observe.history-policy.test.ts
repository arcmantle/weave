...
describe('observe - history policy controls', () => {
	test('maxHistory: keeps only the most recent N records (ring buffer)', () => {
		...
		observe.configure(o, { maxHistory: 3 });
		...
		expect(observe.getHistory(o)).toEqual([]);
	});
	...
	test('reset: returns to pristine state even with maxHistory trimming', () => {
		...
		// take initial pristine snapshot (done automatically on first observe call)
		observe.configure(o, { maxHistory: 2 });
