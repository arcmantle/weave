# Chronicle roadmap and implementation tracker

This document tracks incremental improvements to the deep chronicle feature in `packages/core/library/src/chronicle/chronicle.ts`. We’ll implement one task at a time, review outcomes, and iterate.
...
- API (proposed):
  - `chronicle.beginBatch(object)`, `chronicle.commitBatch(object)`, `chronicle.rollbackBatch(object)`
  - Convenience: `chronicle.batch(object, action)` runs action in a batch, commits on success, rolls back on throw.
- Behavior:
  - Assign a groupId to all change records while batching.
  - `chronicle.undoGroups(object, 1)` reverts the last committed group atomically.
  - Markers/transactions remain supported and compose with batching; `transaction` is implemented on top of `batch`.
  - Optional coalescing for non-batched changes via `chronicle.configure(object, { mergeUngrouped, mergeWindowMs })`.
  - Optional history compaction for repeated sets on the same path via `chronicle.configure(object, { compactConsecutiveSamePath })`.
- Tests (acceptance):
...
- Tests: Added `chronicle.array-delete.test.ts` verifying no holes after delete and that `chronicle.undo` restores the original array.
...
- API (proposed):
  - `chronicle.configure(object, { maxHistory?: number, filter?: (record) => boolean })`
- Outcome:
  - `chronicle.configure` now supports `maxHistory` ring buffer trimming and `filter(record)` for selective history storage.
...
- API: `chronicle.configure(object, { clone?, compare?, diffFilter?: (path) => boolean | 'shallow' })`
- Outcome:
...
- Status: Done
- Goal: chronicle Map/Set mutations using the same proxy mechanics by intercepting method access.
- Outcome:
...
- Tests: `chronicle.map-set.test.ts` covers set/add/delete/clear, listener notifications, batch grouping, and undo/undoGroups.

### 10. Listener quality-of-life options

- Status: Done
- Goal: Improve ergonomics and control.
- API: `chronicle.listen(object, selector, listener, modeOrOptions?, maybeOptions?)`
...
- Tests: `chronicle.listen-options.test.ts` covers once, microtask schedule, debounce, and throttle.
...
- API:
  - `chronicle.transactionAsync(object, async chronicled => { ... })` with auto-commit on resolve and auto-rollback on rejection
  ...
  - Redo stack and helpers:
    - `chronicle.redo(object, steps = 1)` and `chronicle.redoGroups(object, groups = 1)`
    - `chronicle.canUndo(object)`, `chronicle.canRedo(object)`, `chronicle.clearRedo(object)`
- Outcome:
...
- API:
  - `chronicle.onAny(object, listener, options?)` — public global-listener alias; supports the same listener options (once, debounce, throttle, schedule)
  - `chronicle.pause(object)` / `chronicle.resume(object)` — pause queues notifications per-root; resume delivers queued notifications FIFO
  - `chronicle.flush(object)` — deliver queued notifications without resuming
- Outcome:
...
- API: `chronicle.configure(object, { cacheProxies?: boolean })`
- Outcome:
...
- Transactions: return groupId from `chronicle.transaction` and document LIFO semantics for the undo closure.
