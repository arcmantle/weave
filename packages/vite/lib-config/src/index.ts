...
export const libConfig = (
	...
	return async (env: ConfigEnv): Promise<UserConfig> => {
		...
		const entryFiles = entryPatterns
			.flatMap(pattern => globSync(pattern, { cwd: process.cwd() }))
			.filter(file => !/(test|demo|types)\.(ts|tsx)$/.test(file));

		console.log({ entryFiles, cwd: process.cwd() });
		...
		const cfg: UserConfig = {
			...
			build: {
				...
				rollupOptions: {
					...
					output: {
						/** By preserving modules, we retain the folder structure of the
