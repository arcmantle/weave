...
// Local configuration driven by ShowOverlay(cfg)
var startGridN = 3
var keyScheme = "nums"
...
// Overlay appearance (configured via ShowOverlay)
...
// Key mapping: supports various schemes
...
func overlayWindowProc(hwnd uintptr, msg uint32, wParam, lParam uintptr) uintptr {
    ...
    case WM_PAINT:
        ...
        if hdc != 0 {
            ...
            if labelHeight < 12 { labelHeight = 12 }
    ...
    case WM_DESTROY:
            fontCache.mu.Lock()
            for _, f := range fontCache.fonts { if f != 0 { procDeleteObject.Call(f) } }
            fontCache.fonts = nil
            fontCache.mu.Unlock()
            penCache.mu.Lock()
            for _, p := range penCache.pens { if p != 0 { procDeleteObject.Call(p) } }
            penCache.pens = nil
            penCache.mu.Unlock()
            brushCache.mu.Lock()
            for _, b := range brushCache.brushes { if b != 0 { procDeleteObject.Call(b) } }
...
func ensureOverlayClass() error {
    overlayClassOnce.Do(func() {
        ...
        wc := WNDCLASS{ style: 0, lpfnWndProc: windows.NewCallback(overlayWindowProc), hbrBackground: 0, lpszClassName: name }
        if r, _, callErr := procRegisterClassW.Call(uintptr(unsafe.Pointer(&wc))); r == 0 {
            if errno, ok := callErr.(windows.Errno); ok && errno == 1410 {
            } else { overlayClassErr = callErr }
        }
    })
...
func startHotkey(cb func()) (func(), error) {
    ...
    const (
        MOD_ALT=0x0001; MOD_CONTROL=0x0002; HOTKEY_ID=1; VK_C=0x43)
    ...
    if r == 0 {
        ...
        go func() {
            runtime.LockOSThread(); defer runtime.UnlockOSThread(); defer close(done)
            prev := false
            for {
                select { case <-stop: return; default: }
                ctrl,_,_ := procGetAsyncKeyState.Call(0x11)
    ...
    go func(){
        runtime.LockOSThread(); defer runtime.UnlockOSThread(); defer close(done)
        t,_,_ := procGetCurrentThreadId.Call(); tidCh <- t
        var msg MSG
        for {
            ret,_,_ := procGetMessageW.Call(uintptr(unsafe.Pointer(&msg)), 0,0,0)
            if int32(ret) <= 0 { break }
            if msg.message == WM_HOTKEY && msg.wParam == HOTKEY_ID { cb(); continue }
            procTranslateMessage.Call(uintptr(unsafe.Pointer(&msg)))
            procDispatchMessageW.Call(uintptr(unsafe.Pointer(&msg)))
        }
        procUnregisterHotKey.Call(0, HOTKEY_ID)
    }()
    stop := func(){ tid := <-tidCh; procPostThreadMessageW.Call(tid, WM_QUIT, 0,0); <-done; procUnregisterHotKey.Call(0, HOTKEY_ID) }
    return stop, nil
}

func showOverlay() error {
    overlay.mu.Lock(); defer overlay.mu.Unlock()
    if overlay.hwnd != 0 { return nil }
    overlay.doneCh = make(chan struct{})
    overlayPath = nil
    go func(){
        runtime.LockOSThread(); defer runtime.UnlockOSThread(); defer close(overlay.doneCh)
        setDPIAwareness()
        if err := ensureOverlayClass(); err != nil { log.Printf("RegisterClassW failed: %v", err); return }
        mons := enumerateMonitors(); idx := 0
        var pt POINT
        if r,_,_ := procGetCursorPos.Call(uintptr(unsafe.Pointer(&pt))); r != 0 && len(mons) > 0 {
            for i, m := range mons { if pt.x>=m.left && pt.x<m.right && pt.y>=m.top && pt.y<m.bottom { idx=i; break } }
        }
        var m RECT
        if len(mons) > 0 { m = mons[idx] } else {
            cx,_,_ := procGetSystemMetrics.Call(SM_CXSCREEN); cy,_,_ := procGetSystemMetrics.Call(SM_CYSCREEN)
            m = RECT{left:0, top:0, right:int32(cx), bottom:int32(cy)}
        }
        w := uintptr(int(m.right - m.left)); h := uintptr(int(m.bottom - m.top))
        exStyle := uintptr(WS_EX_LAYERED|WS_EX_TRANSPARENT|WS_EX_TOOLWINDOW|WS_EX_TOPMOST|WS_EX_NOACTIVATE)
        style := uintptr(WS_POPUP)
        hwnd, _, err := procCreateWindowExW.Call(exStyle, uintptr(unsafe.Pointer(overlayClassNamePtr)), 0, style, uintptr(m.left), uintptr(m.top), w, h, 0,0,0,0)
        if hwnd == 0 { log.Printf("CreateWindowExW failed: %v", err); return }
        overlay.mu.Lock(); overlay.hwnd = hwnd; overlay.monitors = mons; overlay.monIdx = idx; overlay.mu.Unlock()
        if r,_,err := procSetLayeredWindowAttr.Call(hwnd, 0x000000, 0, LWA_COLORKEY); r == 0 { log.Printf("SetLayeredWindowAttributes failed: %v", err) }
        procSetLayeredWindowAttr.Call(hwnd, 0, uintptr(uint8(overlayAlpha)), LWA_ALPHA)
        ...
        overlay.mu.Lock(); overlay.hwnd = 0; overlay.mu.Unlock()
        close(pollStop)
    }()
...
func hideOverlay() error {
    overlay.mu.Lock(); hwnd := overlay.hwnd; done := overlay.doneCh; overlay.mu.Unlock()
    if hwnd == 0 { return nil }
    const WM_CLOSE = 0x0010
    procPostMessageW.Call(hwnd, WM_CLOSE, 0, 0)
    if done != nil {
        select { case <-done: case <-time.After(2 * time.Second): log.Println("hideOverlay: timeout waiting for window to close") }
    }
    overlay.mu.Lock(); overlayPath = nil; overlay.mu.Unlock()
    return nil
}

func isOverlayVisible() bool {
    overlay.mu.Lock(); defer overlay.mu.Unlock(); return overlay.hwnd != 0
}

func nestedCellCenter(idx int) (int, int, bool) {
    overlay.mu.Lock(); hwnd := overlay.hwnd; pathCopy := make([]int, len(overlayPath)); copy(pathCopy, overlayPath); overlay.mu.Unlock()
    cols, rows := gridDims(); if hwnd == 0 || cols <= 0 || rows <= 0 { return 0,0,false }
    var rc RECT; procGetClientRect.Call(hwnd, uintptr(unsafe.Pointer(&rc)))
    cur := rc
    for _, p := range pathCopy { cur = cellRectForWindowCR(cur, cols, rows, p) }
    w := int(cur.right - cur.left); h := int(cur.bottom - cur.top); if w <= 0 || h <= 0 { return 0,0,false }
    cellW := w / cols; cellH := h / rows
    i := idx - 1; row := i / cols; col := i % cols
    left := int(cur.left) + col*cellW; top := int(cur.top) + row*cellH
    right := int(cur.left) + (col+1)*cellW; bottom := int(cur.top) + (row+1)*cellH
...
func cellRectForWindowCR(r RECT, cols, rows, idx int) RECT {
    ...
    cellW := w / cols; cellH := h / rows
    i := idx - 1; row := i / cols; col := i % cols
    left := int(r.left) + col*cellW; top := int(r.top) + row*cellH
    right := int(r.left) + (col+1)*cellW; bottom := int(r.top) + (row+1)*cellH
